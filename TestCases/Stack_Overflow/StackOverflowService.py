...
class StackOverflowService:
    ...
    def vote_on_post(self, user_id: str, post_id: str, vote_type: VoteType)->None:
        user = self.users[user_id]
        post = self.find_post_by_id(post_id)
        post.vote(user, vote_type)
    
    def accept_answer(self, question_id: str, answer_id: str)->None:
        question = self.questions[question_id]
    ...
    def search_questions(self, strategies: List[SearchStrategy])->List[Question]:
        res = list(self.questions.values())

        for strategy in strategies:
            res = strategy.filter(res)
